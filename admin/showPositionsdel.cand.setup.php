	...
	if (!isset($_POST['ChosenElection'])) {
		...
		if ($chosenElection == 'default') {
			...
			if (mysqli_num_rows($query) < 1) {
				...
				echo "<br><br>";
				echo "<button id='proceedPositiondel' class='btn btn-default btn-lg pull-right'>Next</button>";
			}
		}
...
<script>
	$(document).ready(function() {
		$("#proceedPositiondel").click(function() {
			var chosenPosition = $("#currentPositions").val();
			var hiddenElection = $("#hiddenElection").val();
			$("#getInputsdel").load("loadGetCandsdel.cand.setup.php",
				{
					position: chosenPosition,
					election: hiddenElection
				}).hide().fadeIn(1500);
			$("#proceedPositiondel").hide(1000);
		});
	});
</script>